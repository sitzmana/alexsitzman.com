...
| `PortfolioContent` | Everything, plus derived views (`NavPages`, `FeaturedProjects`) |
...
Primitives/         ContentSection, TagList, LinkList, IconGlyph
...
2. Clear `dist/`, clearing read-only attributes as it goes (D14).
3. Concatenate and hash assets.
